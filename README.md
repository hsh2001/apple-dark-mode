...
## 왜 다크모드를 사용해야 할까?
우리는 왜 다크모드를 알고 사용해야 할까요? 요즘 유행이라서? 멋져 보여서? 여기 그보다 더 큰 이유 몇 가지가 있습니다.

아름다운 웹 페이지의 디자인은 중요하지만 우리에게 가장 중요한 목적은 웹 페이지의 정보를 사용자에게 명확하게 전달하는 것입니다. 영화관 직원들은 영화가 시작하기 전 가장 먼저 상영관의 조명을 꺼 주변을 어둡게 만듭니다. 주변을 꾸미는 요소가 너무 화려하거나 눈에 띄면 중심 콘텐츠에 대한 집중도는 외부로 분산되기 쉽습니다. 다크모드를 비롯한 여러 어두운 환경은 주변 요소보다 중심 콘텐츠에 더욱 집중할 수 있도록 해줍니다. 요즘의 심플한 UI 디자인 패러다임도 이와 같은 맥락을 갖습니다.

개발자는 사용자의 장애 여부와 관계없이 모든 사용자가 편리하게 소프트웨어를 사용할 수 있도록 제공할 의무를 가지고 있습니다. 애플의 'VoiceOver'는 시각 장애인을 위한 애플의 대표적인 접근성 도구이며 이는 많은 시각장애인들이 아이폰을 선택하는 이유가 되었습니다. 저시력자나 밝은 빛에 예민한 사용자를 위해 화면 전체를 반전시키는 '색 반전' 기능도 많은 이들에게 도움을 주었습니다. 다크모드 또한 저시력자나 밝은 빛에 예민한 사용자에게 눈의 피로를 덜어주는 효과를 주어 큰 도움이 됩니다. 사용자가 색 반전 기능을 켜고 다크모드 웹 페이지에 방문하면 다시 밝은 색이 되지 않을까 걱정된다면, 괜찮습니다. 애플의 '스마트 반전' 기능은 상황에 따라 적절하게 화면을 반전시키는 기능으로, 이미 다크모드를 지원하고 있다면 화면을 반전시키지 않습니다.
